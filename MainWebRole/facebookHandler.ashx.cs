...
namespace MOOPFramework
{
  ...
  public class facebookHandler : IHttpHandler
  {
    ...
    public void ProcessRequest(HttpContext context)
    {
      ...
      FrameworkUtility.ParsedURI pUri = new FrameworkUtility.ParsedURI(context.Request.RawUrl.Replace(":20000", ""));
      ...
      if (fbApiKey != string.Empty)
      {
        ...
        if (context.Request.Params["signed_request"] != null)
        {
          ...
          if (nvc.Count > 0)
          {
            ...
            retVal.Append(queueFacebookMessage(fbUserID, fbApplicationID, oAuthToken, fbIssuedAt, fbExpires, fbProcessingQueue,azureAccount, azureSharedKey));
          }
        }
        if (fbUserID == string.Empty || fbUserID == "fb")
        {
          if (!context.Request.Params["userid"].StartsWith("fb"))
            fbUserID = "fb" + context.Request.Params["userid"];
          oAuthToken = context.Request.Params["gazinga"];
          retVal.Append(queueFacebookMessage(fbUserID, fbApplicationID, oAuthToken, DateTime.UtcNow, DateTime.UtcNow.AddMinutes(60), fbProcessingQueue, azureAccount, azureSharedKey));
        }
      }
